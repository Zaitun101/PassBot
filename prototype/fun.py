...
def start(message):
    dal.create_db()
    if (message.chat.id != var.adm_chat_id and message.chat.id != var.secutity_chat_id):
    ...
    if len(records) == 0:
        ...
        for row in records:
            bot.send_message(message.chat.id,
                             i18n.t('text.today_request', rowid=row[0], fio=row[2], guest_fio=row[3], date=row[4],
...
def add_visit_time(message, date, fio, guest_fio):
    ...
    if (re.fullmatch("\d{1,2}[:][0-5][0-9]", time)):
        if 7 < int(time.split(':')[0]) and int(time.split(':')[0]) < 19:
            tmp = 'ФИО: ' + str(fio) + '\nФИО гостя: ' + str(guest_fio) + '\nДата визита: ' + str(
                date) + '\nВремя визита: ' + str(time)
            msg = bot.send_message(message.chat.id, f"Проверь данные, все верно?\n{tmp}",
                                   reply_markup=basicui.create_buttons(["Все верно", i18n.t('text.change_request')]))
...
def check_db(message, fio, guest_fio, date, time):
    if message.text == i18n.t('text.change_request'):
        ...
        tmp = "Номер заявки: " + request_number + '\nФИО: ' + str(fio) + '\nФИО гостя: ' + str(
            guest_fio) + '\nДата визита: ' + date + '\nВремя визита: ' + time
        send_notif_adm(tmp, request_number, message)
...
def send_notif_adm(tmp, requestNumber, message):
    markup_inline = types.InlineKeyboardMarkup()
    btn_yes = types.InlineKeyboardButton(text="Подтвердить", callback_data='1,' + str(requestNumber))
    btn_no = types.InlineKeyboardButton(text="Отменить", callback_data='2,' + str(requestNumber))
    markup_inline.add(btn_yes, btn_no)
    bot.send_message(var.adm_chat_id, f"Пришла новая заявка!\n{tmp}", reply_markup=markup_inline)
